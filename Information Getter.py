...
|___|_| |_|___/\__\__,_|____/ \___/_/\_\
                                           {M} Tool: InstaBox
                      {M} Author: Club Rocket Power
                      {M} Github: Github.com/r0cketp0wer
                      {M} Twitter: NRocketmann
                      {M} Instagram: rocketman_mega
							 	
		     {U}{Y}~ {G}Information Getter {M}- {G}By Club Rocket Power {Y}~{UR}'''.format(Y=YELLOW,M=MAGENTA,G=GREEN,GR=GRAY,C=CYAN,U=UNDERLINE,UR=RUNDERLINE)
...
def Clear():
	if os.name == 'nt':
		os.system('cls')
		os.system('title Information getter by Club Rocket Power')
	else:
		os.system('clear')
